...
```
在项目中会有很多的模块都需要发送网络请求，常见的比如登录模块，首页模块等，
如果我们项目中直接使用诸如 axios.get(), axios.post()，会存在很多弊端:
首先这样做会导致我们每个模块对axios依赖性太强，意味着我们项目中的每个模块都和一个第三方库耦合度较高
导致后期如果axios不在维护，我们要更换库的时候将非常麻烦；
其次，在我们发送网络请求的时候，会有很多共同的特性，比如，在我们成功登录之后的其他请求中，往往需要在请求头中添加token，然后发送请求；
在每次请求中，我们想展示一个loading... 这些功能如果在每次请求的逻辑中都写一遍，很明显，代码重复度太高了。
```
